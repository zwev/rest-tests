...
select_settings = {'select_session': 'S2','query_session': 'S2','select_action': '000','query_target': 'A','sel_flag': 'All'}
...
def invstop(args, ver):
    ...
    for item in response_body:
        ...
        for x in item["count"]:
            totalcount += x
    tagcounts = totalcount/invtime/len(response_body)
    print("{} tag reads per second".format(tagcounts))
    check.less_equal(tagcounts, 3, "3 max")
    check.greater_equal(tagcounts, 0, "0 min")
...
def test_func5(ip, serial, tout, do_reset, ssl, ver):
