...
# After posting and verifying that the settings are correct, an inventory is run and read rates are checked to be within the expected range for fast mode.
...
def invstop(args, ver):
    ...
    if epic == True:
        ...
        check.less_equal(total_reads/invtime, 1000, "1000 max")
        check.greater_equal(total_reads/invtime, 500, "500 min")
        tags.clear()
...
def test_epicfast(ip, serial, tout, do_reset, ssl, ver):
